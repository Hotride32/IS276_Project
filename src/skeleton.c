...
//void skeleton_draw(Entity *self);
void skeleton_think(Entity *self);
void skeleton_update(Entity *self);
int  skeleton_touch(Entity *self,Entity *other);
int  skeleton_damage(Entity *self,int amount, Entity *source);
void skeleton_die(Entity *self);
int skeleton_player_sight_check(Entity *self);
void skeleton_think_hunting(Entity *self);
void skeleton_turn(Entity *self,int dir);
...
Entity *skeleton_spawn(Vector2D position)
{
    ...
    ent->sprite = gf2d_sprite_load_all("images/skeleton-removebg-preview.png", 128, 64, 4);
    ...
    ent->frameRate = 0.1;
    ent->frameCount = 2;
    ent->think = skeleton_think_patroling;
    //ent->think = skeleton_think_hunting;
    //ent->draw = skeleton_draw;
    ...
    ent->shape = gf2d_shape_rect(32, 0, 64, 55);
    gf2d_body_set(
        &ent->body,
        "skeleton",
        1,
    ...
    return ent;
    //return skeleton_new(position,"actors/space_bug.actor");
}
...
/*Entity *skeleton_new(Vector2D position,char *actorFile)
{
    ...
    //gf2d_line_cpy(self->name,"skeleton");
    //self->parent = NULL;
   ...
   // vector2d_copy(self->scale,self->actor.al->scale);
    ...
    self->think = skeleton_think_patroling;
    self->draw = skeleton_draw;
    self->update = skeleton_update;
...
void skeleton_think_attacking(Entity *self)
{
    ...
    if (self->frame >= 14) {
        ...
        self->think = skeleton_think_hunting;
...
void skeleton_think_patroling(Entity *self)
{
    ...
    if (skeleton_player_sight_check(self))
    {
        self->think = skeleton_think_hunting;
        return;
    }
...
void skeleton_think_hunting(Entity *self)
{
    ...
    //skeleton loses sight of player
    if (vector2d_magnitude_compare(vector2d(self->position.x - player->position.x,self->position.y - player->position.y),150) > 0)
    {
        ...
        self->think = skeleton_think_patroling;// idle think
        //gf2d_actor_set_action(&self->actor,"walk");
        return;
    }
    //skeleton gets in range of player
...
{
    Vector2D dir = {0};
    slog("skeleton taking %i damage!",amount);
    self->health -= amount;
