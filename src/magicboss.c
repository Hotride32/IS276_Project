...
//void magicBoss_draw(Entity *self);
void magicBoss_think(Entity *self);
void magicBoss_update(Entity *self);
int  magicBoss_touch(Entity *self,Entity *other);
int  magicBoss_damage(Entity *self,int amount, Entity *source);
void magicBoss_die(Entity *self);
int magicBoss_player_sight_check(Entity *self);
void magicBoss_think_hunting(Entity *self);
void magicBoss_turn(Entity *self,int dir);
void magicBoss_think_patroling(Entity *self);
...
Entity *magicBoss_spawn(Vector2D position)
{
    ...
    ent->sprite = gf2d_sprite_load_all("images/mage-2-122x110.png", 122,110, 4);
    ...
    //ent->touch = magicBoss_touch;
    ent->damage = magicBoss_damage;
    //ent->die = magicBoss_die;
    ent->health = 150;
    ent->maxHealth = 150;
    ent->str = 50;
    ent->attackcool = 0;
    ...
    ent->shape = gf2d_shape_rect(0, 0, 122, 110);
    gf2d_body_set(
        &ent->body,
        "magicBoss",
        1,
    ...
    return ent;
    //return magicBoss_new(position,"actors/space_bug.actor");
}
...
/*Entity *magicBoss_new(Vector2D position,char *actorFile)
{
    ...
    //gf2d_line_cpy(self->name,"magicBoss");
    //self->parent = NULL;
   ...
   // vector2d_copy(self->scale,self->actor.al->scale);
    ...
    self->think = magicBoss_think_patroling;
    self->draw = magicBoss_draw;
    self->update = magicBoss_update;
    self->touch = magicBoss_touch;
    self->damage = magicBoss_damage;
    self->die = magicBoss_die;
...
void magicBoss_think_attacking(Entity *self)
{
    ...
    if (self->attackcool == 0) {
        ...
        Entity* laser;
        if (random == 1) {
            laser = laser_spawn(vector2d(player->position.x + 230, player->position.y - 16), self->flip, PLAYER_LAYER);
        ...
        */
        self->think = magicBoss_think_hunting;
...
void magicBoss_think_patroling(Entity *self)
{
    ...
    if (magicBoss_player_sight_check(self))
    {
        self->think = magicBoss_think_hunting;
        return;
    }
    ...
    {
        magicBoss_turn(self,self->facing.x * -1);
    }*/
...
void magicBoss_think_hunting(Entity *self)
{
    ...
    //magicBoss loses sight of player
    if (vector2d_magnitude_compare(vector2d(self->position.x - player->position.x,self->position.y - player->position.y),500) > 0)
    {
        ...
        self->think = magicBoss_think_patroling;// idle think
        //gf2d_actor_set_action(&self->actor,"walk");
        return;
    }
    //magicBoss gets in range of player
    if ((vector2d_magnitude_compare(vector2d(self->position.x - player->position.x,self->position.y - player->position.y),300) < 0) && self->attackcool == 0)
    {
    ...
    if (player->position.x+50 < self->position.x)
    {
        skull_turn(self, -1);
        self->velocity.x = -0.5;
    }
    ...
    if (player->position.y-30 < self->position.y)
    {
        //skull_turn(self, 1);
        self->velocity.y = -0.05;
    }
...
{
    Vector2D dir = {0};
    slog("magicBoss taking %i damage!",amount);
    self->health -= amount;
