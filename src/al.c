...
//void al_draw(Entity *self);
void al_think(Entity *self);
void al_update(Entity *self);
int  al_touch(Entity *self,Entity *other);
int  al_damage(Entity *self,int amount, Entity *source);
void al_die(Entity *self);
int al_player_sight_check(Entity *self);
void al_think_hunting(Entity *self);
void al_turn(Entity *self,int dir);
void al_think_patroling(Entity *self);
...
Entity *al_spawn(Vector2D position)
{
    ...
    ent->shape = gf2d_shape_rect(0, 0, 64, 56);
    gf2d_body_set(
        &ent->body,
        "Al",
        1,
    ...
    return ent;
    //return al_new(position,"actors/space_bug.actor");
}
...
/*Entity *al_new(Vector2D position,char *actorFile)
{
   ...
   // vector2d_copy(self->scale,self->actor.al->scale);
    ...
    self->think = al_think_patroling;
    self->draw = al_draw;
    self->update = al_update;
    self->touch = al_touch;
    self->damage = al_damage;
    self->die = al_die;
...
void al_think_attacking(Entity *self)
{
    ...
    //Entity* laser;
    if ((vector2d_magnitude_compare(vector2d(self->position.x - player->position.x, self->position.y - player->position.y), 50) < 0)) {
        self->frameAnimStart = 41;
        if (self->frame < 41) {
            self->frame = 41;
        }
        //i++;
        ...
        if (self->frame >= 43) {
            ...
            s = gf2d_shape_rect(self->position.x + (self->flip.x * -48) - 16, self->position.y, 70, 40);
        ...
        if (self->jumpcool == 0) {
            self->velocity.y = -3;
            self->velocity.x = 3 * self->facing.x;
            self->jumpcool = 20;
        }

        

        self->frameAnimStart = 44;
        if (self->frame < 44) {
            self->frame = 44;
        }
        //i++;
        ...
        if (self->frame >= 46) {
            ...
            Collision* c;
            List* collisionList = NULL;
            s = gf2d_shape_rect(self->position.x + (self->flip.x * -48) - 16, self->position.y, 50, 50);
            collisionList = entity_get_clipped_entities(self, s, PLAYER_LAYER, 0);
            count = gfc_list_get_count(collisionList);
            //slog("hit %i targets", count);
            for (i = 0; i < count; i++)
...
void al_think_patroling(Entity *self)
{
    ...
    {
        al_turn(self,self->facing.x * -1);
    }*/
    
    
    //Entity* player = player_get();

    if (self->spawnPos.x -30 > self->position.x)
    {
        al_turn(self, 1);
    ...
    self->velocity.x = (1 * self->facing.x)/2;
...
void al_think_hunting(Entity *self)
{
    ...
    if (vector2d_magnitude_compare(vector2d(self->position.x - player->position.x,self->position.y - player->position.y),300) > 0)
    {
        ...
        self->think = al_think_patroling;// idle think
        //gf2d_actor_set_action(&self->actor,"walk");
        return;
    }
    //al gets in range of player
...
{
    Vector2D dir = {0};
    slog("al taking %i damage!",amount);
    self->health -= amount;
